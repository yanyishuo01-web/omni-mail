...
NAVER 可能针对 Cloudflare Worker 的动态 TCP 出口触发异常登录或自动停用 IMAP。建议使用
可随时撤销应用密码的专用测试账号，从实际部署的 Worker 验证登录、正文、附件、已读、
凭据撤销/恢复，并以 15 分钟周期观察至少 24 小时。
...
系统设置只控制入口显示。隐藏入口不会删除 D1 中已有账号、密文或索引，也不会停止已有账号
的后台同步或禁用 API。已保存的隐藏设置在升级后继续保留；需要停止某个账号的同步时，请断开该账号。
